...
def main():
    ...
    for class_idx, class_name in enumerate(classes):
        ...
        files = sorted(f for f in os.listdir(class_dir) if f.lower().endswith(".wav"))
        ...
        for i, fname in enumerate(files):
            ...
            try:
                ...
                windowed = extract_loudest_window(waveform, WINDOW_SAMPLES, TARGET_SR)

                scores, embeddings, spectrogram = yamnet(windowed)
                # Mean-only pooling can erase a short gunshot/scream inside a
                # mostly quiet 2.5s window.  Mean+max+std retains both clip
                # context and transient information (3072 features total).
                clip_embedding = pool_embedding_frames(embeddings.numpy())

                all_embeddings.append(clip_embedding)
                all_labels.append(class_idx)
                all_groups.append(f"{class_name}/{get_group_id(fname)}")
                all_paths.append(f"{class_name}/{fname}")
