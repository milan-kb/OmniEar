...
export function IncidentDrawer({
  ...
  if (!alert || !meta || !labelMeta) return null;

  return (
    <aside
      role="dialog"
      aria-label={`Incident ${alertId}`}
      className="glass-raised incident-drawer-enter fixed inset-y-0 right-0 z-[60] flex w-full max-w-[420px] flex-col overflow-y-auto"
    >
      <header className="flex items-start gap-3 border-b border-white/10 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-[11px] text-muted-foreground">Routed to {labelMeta.routeTo}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
